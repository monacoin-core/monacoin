...
Monacoin Core version 25.0 is now available from:
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/monacoin-core/monacoin/issues>
...
installer (on Windows) or just copy over `/Applications/Monacoin-Qt` (on macOS)
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Monacoin
...
- All JSON-RPC methods accept a new [named
  ...
  python test framework and `monacoin-cli` tool both take advantage of this, so
  for example:

```sh
monacoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
monacoin-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Monacoin Core will now return an error at startup. (#25574)
...
before Monacoin Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching monacoind with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/monacoin-core/monacoin/blob/master/contrib/verify-binaries/README.md
  (#27358)
