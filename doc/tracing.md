# User-space, Statically Defined Tracing (USDT) for Monacoin Core

Monacoin Core includes statically defined tracepoints to allow for more
...
**Note**: Monacoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Monacoin Core
...
Multiple tools can list the available tracepoints in a `monacoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Monacoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/monacoind
stap net        outbound_message 0x0000000000107c05 /src/monacoind
stap validation block_connected  0x00000000002fb10c /src/monacoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Monacoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/monacoind | grep NT_STAPSDT -A 4 -B 2
