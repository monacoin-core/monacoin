...
class AbandonConflictTest(MonacoinTestFramework):
    ...
    def run_test(self):
        ...
        # Verify that B and C's 10 MONA outputs are available for spending again because AB1 is now conflicted
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance + Decimal("20"))
        balance = newbalance

        # There is currently a minor bug around this and so this test doesn't work.  See Issue #7315
        # Invalidate the block with the double spend and B's 10 MONA output should no longer be available
