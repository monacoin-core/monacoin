...
class MonacoinTestMetaClass(type):
    """Metaclass for MonacoinTestFramework.

    Ensures that any attempt to register a subclass of `MonacoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'MonacoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("MonacoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("MonacoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class MonacoinTestFramework(metaclass=MonacoinTestMetaClass):
    ...
    def setup(self):
        ...
        fname_monacoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "monacoind" + config["environment"]["EXEEXT"],
        )
        fname_monacoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "monacoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_monacoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "monacoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.monacoind = os.getenv("MONACOIND", default=fname_monacoind)
        self.options.monacoincli = os.getenv("MONACOINCLI", default=fname_monacoincli)
        self.options.monacoinutil = os.getenv("MONACOINUTIL", default=fname_monacoinutil)
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("MonacoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'monacoind', self.options.monacoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'monacoin-cli', self.options.monacoincli) for v in versions]
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as monacoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    monacoind=self.options.monacoind,
                    monacoin_cli=self.options.monacoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in monacoin.conf
    ...
    def skip_if_no_monacoind_tracepoints(self):
        """Skip the running test if monacoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("monacoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_monacoind_zmq(self):
        """Skip the running test if monacoind has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if monacoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("monacoin-wallet has not been compiled")

    def skip_if_no_monacoin_util(self):
        """Skip the running test if monacoin-util has not been compiled."""
        if not self.is_monacoin_util_compiled():
            raise SkipTest("monacoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if monacoin-cli has not been compiled."""
    ...
    def is_cli_compiled(self):
        """Checks whether monacoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether monacoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_monacoin_util_compiled(self):
        """Checks whether monacoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_MONACOIN_UTIL")
