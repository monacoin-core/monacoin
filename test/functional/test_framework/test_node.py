...
MONACOIND_PROC_WAIT_TIMEOUT = 60
...
class TestNode():
    ...
    def __init__(self, i, datadir, *, chain, rpchost, timewait, timeout_factor, monacoind, monacoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        """
        Kwargs:
        ...
        self.monacoinconf = os.path.join(self.datadir, "monacoin.conf")
        self.stdout_dir = os.path.join(self.datadir, "stdout")
        self.stderr_dir = os.path.join(self.datadir, "stderr")
        self.chain = chain
        self.rpchost = rpchost
        self.rpc_timeout = timewait
        self.binary = monacoind
        ...
        # Configuration for logging is set as command-line args rather than in the monacoin.conf file.
        # This means that starting a monacoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(monacoin_cli, self.datadir)
    ...
    def __del__(self):
        # Ensure that we don't leave any monacoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time monacoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by monacoind, and
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; monacoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to monacoind after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; monacoind is still starting
    ...
    def wait_until_stopped(self, timeout=MONACOIND_PROC_WAIT_TIMEOUT):
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'monacoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'monacoind should have exited within {self.rpc_timeout}s '
    ...
    def add_p2p_connection(self, p2p_conn, *, wait_for_verack=True, **kwargs):
        ...
        if wait_for_verack:
            ...
            # Consistency check that the Monacoin Core has received our user agent string. This checks the
            # node's newest peer. It could be racy if another Monacoin Core node has connected since we opened
...
class TestNodeCLI():
    ...
    def __init__(self, binary, datadir):
        ...
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.monacoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with monacoin-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        ...
        self.log.debug("Running monacoin-cli {}".format(p_args[2:]))
