...
Tracing (USDT) support for Monacoin Core on Linux using. For more information on
USDT support in Monacoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `monacoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `monacoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Monacoin Core USDT documentation] on how to list available tracepoints in your
`monacoind` binary.

[Monacoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/monacoind
```
...
Monacoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/monacoind
```
...
In a different terminal, starting Monacoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/monacoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/monacoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/monacoind
```
