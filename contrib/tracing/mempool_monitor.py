...
""" Example logging Monacoin Core mempool events using the mempool:added,
...
def main(monacoind_path):
    monacoind_with_usdts = USDT(path=str(monacoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    monacoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    monacoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    monacoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    monacoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[monacoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/monacoind")
        exit(1)
