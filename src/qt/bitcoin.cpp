...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<MonacoinUnit>("MonacoinUnit");
#else
    qRegisterMetaType<MonacoinUnit>("MonacoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. monacoin_de.qm (shortcut "de" needs to be defined in monacoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. monacoin_de_DE.qm (shortcut "de_DE" needs to be defined in monacoin.qrc)
...
MonacoinApplication::MonacoinApplication()
...
void MonacoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the MonacoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", MonacoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void MonacoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool MonacoinApplication::createOptionsModel(bool resetSettings)
{
...
void MonacoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new MonacoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &MonacoinGUI::quitRequested, this, &MonacoinApplication::requestShutdown);
...
void MonacoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void MonacoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool MonacoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void MonacoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &MonacoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &MonacoinApplication::handleRunawayException);
    connect(this, &MonacoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &MonacoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void MonacoinApplication::parameterSetup()
{
...
void MonacoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void MonacoinApplication::requestInitialize()
{
...
void MonacoinApplication::requestShutdown()
{
...
void MonacoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // monacoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MonacoinGUI::handlePaymentRequest);
            connect(window, &MonacoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void MonacoinApplication::handleRunawayException(const QString &message)
{
...
void MonacoinApplication::handleNonFatalException(const QString& message)
{
...
WId MonacoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool MonacoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MonacoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(monacoin);
    Q_INIT_RESOURCE(monacoin_locale);
    ...
    /// 6-7. Parse monacoin.conf, determine network, switch to network specific
    ...
    // monacoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
