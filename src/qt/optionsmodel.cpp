...
/** Call node.updateRwSetting() with Monacoin 22.x workaround. */
...
//! Convert enabled/size values to monacoin -prune setting.
...
//! Get pruning enabled value to show in GUI from monacoin -prune setting.
...
//! Get pruning size value to show in GUI from monacoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayMonacoinUnit")) {
        settings.setValue("DisplayMonacoinUnit", QVariant::fromValue(MonacoinUnit::MONA));
    }
    QVariant unit = settings.value("DisplayMonacoinUnit");
    if (unit.canConvert<MonacoinUnit>()) {
        m_display_monacoin_unit = unit.value<MonacoinUnit>();
    } else {
        m_display_monacoin_unit = MonacoinUnit::MONA;
        settings.setValue("DisplayMonacoinUnit", QVariant::fromValue(m_display_monacoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if monacoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_monacoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<MonacoinUnit>() == m_display_monacoin_unit) return;
    m_display_monacoin_unit = new_unit.value<MonacoinUnit>();
    QSettings settings;
    settings.setValue("DisplayMonacoinUnit", QVariant::fromValue(m_display_monacoin_unit));
    Q_EMIT displayUnitChanged(m_display_monacoin_unit);
}
