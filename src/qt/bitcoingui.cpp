// Copyright (c) 2011-2022 The Monacoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/monacoingui.h>
...
const std::string MonacoinGUI::DEFAULT_UIPLATFORM =
...
MonacoinGUI::MonacoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &MonacoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &MonacoinGUI::showModalOverlay);
...
void MonacoinGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &MonacoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &MonacoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &MonacoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Monacoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Monacoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Monacoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Monacoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a monacoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Monacoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &MonacoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &MonacoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &MonacoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &MonacoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &MonacoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &MonacoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &MonacoinGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &MonacoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &MonacoinGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &MonacoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &MonacoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &MonacoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &MonacoinGUI::showDebugWindow);
}

void MonacoinGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void MonacoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &MonacoinGUI::setCurrentWalletBySelectorIndex);
...
void MonacoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &MonacoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &MonacoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &MonacoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &MonacoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void MonacoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void MonacoinGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &MonacoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &MonacoinGUI::removeWallet);
...
WalletController* MonacoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void MonacoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &MonacoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &MonacoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &MonacoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &MonacoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &MonacoinGUI::incomingTransaction);
    connect(this, &MonacoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void MonacoinGUI::removeWallet(WalletModel* walletModel)
{
...
void MonacoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void MonacoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void MonacoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MonacoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void MonacoinGUI::createTrayIcon()
{
...
void MonacoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &MonacoinGUI::toggleHidden);
...
void MonacoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void MonacoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void MonacoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void MonacoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void MonacoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void MonacoinGUI::openClicked()
{
...
void MonacoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MonacoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MonacoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MonacoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MonacoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MonacoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void MonacoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void MonacoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Monacoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void MonacoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MonacoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void MonacoinGUI::updateHeadersSyncProgressLabel()
{
...
void MonacoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void MonacoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &MonacoinGUI::quitRequested);
...
void MonacoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void MonacoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void MonacoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &MonacoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &MonacoinGUI::show);
                e->ignore();
            }
        }
    }
...
void MonacoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MonacoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MonacoinGUI::incomingTransaction(const QString& date, MonacoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(MonacoinUnits::formatWithUnit(unit, amount, true));
...
void MonacoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MonacoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MonacoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MonacoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MonacoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void MonacoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MonacoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void MonacoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void MonacoinGUI::updateWindowTitle()
{
...
void MonacoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MonacoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MonacoinGUI::detectShutdown()
{
...
void MonacoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void MonacoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MonacoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = MonacoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void MonacoinGUI::subscribeToCoreSignals()
{
...
void MonacoinGUI::unsubscribeFromCoreSignals()
{
...
bool MonacoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<MonacoinUnit> units = MonacoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MonacoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, MonacoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const MonacoinUnit u : MonacoinUnits::availableUnits()) {
        menu->addAction(MonacoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(MonacoinUnit newUnits)
{
    setText(MonacoinUnits::longName(newUnits));
}
