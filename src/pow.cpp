...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight < params.nSwitchLyra2REv2_DGW + PastBlocksMin) {
        return UintToArith256(params.powLimit).GetCompact();
    }

    for (unsigned int i = 1; BlockReading && BlockReading->nHeight >= params.nSwitchLyra2REv2_DGW; i++) {
        if (PastBlocksMax > 0 && i > PastBlocksMax) { break; }
        CountBlocks++;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if(pindexLast->nHeight+1 >= params.nSwitchLyra2REv2_DGW)
    {
        // DGWv3
        return DarkGravityWave(pindexLast, pblock, params);
    }
    else if(pindexLast->nHeight+1 >= params.nSwitchKGWblock && pindexLast->nHeight+1 < params.nSwitchDIGIblock){
        // KGW
        return GetNextWorkRequired_V2(pindexLast, pblock, params);
    }

    int64_t adjustmentInterval = params.DifficultyAdjustmentInterval();
    if ((pindexLast->nHeight+1) >= params.nSwitchDIGIblock) {
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    bool fNewDifficultyProtocol = ((pindexLast->nHeight+1) >= params.nSwitchDIGIblock);
