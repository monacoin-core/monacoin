...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(nHeight >= consensusParams.nSwitchLyra2REv2_DGW), block.nBits, consensusParams))
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        LOCK(cs_main);
                        int nHeight = m_blockman.LookupBlockIndex(hash)->nHeight;
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, nHeight, params.GetConsensus())) {
